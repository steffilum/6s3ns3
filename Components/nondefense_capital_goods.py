...
df = get_most_recent_series_of_date("ANDENO", "2020-01-01", fred)
...
model = arch_model(pct_chg_nondefense_capital_goods, mean = 'AR', lags = 1, vol='Garch', p=1, q=1)
fit = model.fit()
# # print(fit.summary())

last_month = pct_chg_nondefense_capital_goods.index[-1]+ pd.offsets.MonthBegin(1)
...
pct_chg_pred = pd.concat([pct_chg_nondefense_capital_goods, new_df])
