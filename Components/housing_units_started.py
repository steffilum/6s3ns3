...
print("ADF Test Result: ", adfuller(pct_chg_housing_units_started, regression="c"))
...
end_date_pred = pd.Period(get_date, freq='Q').end_time.to_period(freq='M').start_time
...
def quart_pct_chg_housing_units_started(given_date = "2020-01-01", period = 'Q'):
    ...
    model = ARIMA(pct_chg_housing_units_started, order=(1, 0, 1), trend = 'c', freq = 'MS').fit(start_params = np.full(1+1+1, .01))
    start_date_pred = pct_chg_housing_units_started.index[-1]+ pd.offsets.MonthBegin(1)
